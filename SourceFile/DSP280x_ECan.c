...
/* Configure eCAN RX and TX pins for CAN operation using eCAN regs*/
    ...
    ECanaShadow.CANTIOC.all = ECanaRegs.CANTIOC.all;//TX（I/O）控制寄存器
    ECanaShadow.CANTIOC.bit.TXFUNC = 1;//CANTX引脚被用于CAN传输功能
...
/* Configure eCAN for HECC mode - (reqd to access mailboxes 16 thru 31) */
    ...
    ECanaShadow.CANMC.bit.SCB = 1;//SCC兼容模式位，该位在SCC模式下保留
...
//-----------------------------------------------------------------
//MSGCTRL 消息控制寄存器  作为CAN模块的初始化过程的一部分，MSGCTRLn寄存器的所有
//位都必须先初始化为0，然后再将不同的位字段初始化为所需的值。
...
//--------------------------------------------------------
//CANBTC 位时间配置寄存器 在使用CAN模块之前必须对该寄存器进行编程
//并且只能在初始化模式中写入
...
//100MHZ系统时钟的位配置参数  不同的CAN模拟时钟、比特率和采用点
//eCAN模块中，CAN总线一个时间位的长度由参数TSEG1和TSEG2和BRP设定，可计算出波特率
    ...
    #endif
//---------------------------------------------------
//SAM CAN模块用该参数来设置采样数目，所有CAN总线上的控制器都必须有相同的波特率和位时间长度
//在各个控制器的不同时钟频率下，波特率要根据该参数调节
    ECanaShadow.CANBTC.bit.SAM = 1;
    ECanaRegs.CANBTC.all = ECanaShadow.CANBTC.all;

    ECanaShadow.CANMC.all = ECanaRegs.CANMC.all;
    ECanaShadow.CANMC.bit.CCR = 0 ;        //请求正常模式，CCR位置0
    ECanaRegs.CANMC.all = ECanaShadow.CANMC.all;
